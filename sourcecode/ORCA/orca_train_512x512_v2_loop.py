import os      ### Obs.: tentativa de implementar um loop para executar todos os augs (incompleto ainda)
import sys
...
# paper_augmentations = [None, "horizontal_flip", "vertical_flip", "rotation", "transpose", "grid_distortion",
#                        "optical_distortion", "color_transfer", "inpainting"]
#
...
list_loss           = ['BCELoss', 'L1Loss']
list_optimizer      = ['Adam', 'Adadelta']
list_strategy       = ['no_augmentation', 'color_augmentation', 'inpainting_augmentation', 'geometric', 'distortion',
                       'standard', 'random', 'solo']
paper_augmentations = [None, "horizontal_flip", "vertical_flip", "rotation", "transpose", "grid_distortion",
                       "optical_distortion", "color_transfer", "inpainting"]
...
for loss_function in list_loss:
    for optimizer_algorithm in list_optimizer:
        for augmentation_strategy in list_strategy:
            if augmentation_strategy in ["standard", "random"]:
            ...
            for i in range(0, nloop):
                dataset_name = "{}_{}".format(name_id, dataset)
                name_id = name_id + 1
